...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // TESTE 6: Verifique se, ao adicionar os pedidos, dentre bebidas e comidas, o array `objetoRetornado.consumption` contém os itens pedidos.
    objetoRetornado.order("agua");
    objetoRetornado.order("sopa");
    objetoRetornado.order("cerveja");
    expect(objetoRetornado.consumption).toEqual(["coxinha", "agua", "sopa", "cerveja"]);
    // TESTE 7: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.
    objetoRetornado.order('coxinha');
    objetoRetornado.order('agua');
    expect(objetoRetornado.consumption).toEqual(["coxinha", "agua", "sopa", "cerveja", "coxinha", "agua"]);
    // TESTE 8: Verifique se, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`+ 10%
    expect(objetoRetornado.pay()).toBe(35.64);
  });
});
